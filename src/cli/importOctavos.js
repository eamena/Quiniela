...
// Each participant block in the octavos spreadsheet is 5 columns wide:
//   col+0 = home team, col+1 = away team, col+2 = pred_home, col+3 = pred_away, col+4 = empty
const PARTICIPANT_BLOCK_SIZE = 5;

// Offset above 16avos (10000) so octavos matches never conflict with earlier bracket rows
const MATCH_ROW_OFFSET = 20000;
...
function importBracketOctavos(filePath) {
  ...
  if (!participants.length)
    throw new Error("No participants found in octavos.xlsx.");
...
initDb();
const filePath = process.env.BRACKET_FILE || "octavos.xlsx";
const result = importBracketOctavos(filePath);
