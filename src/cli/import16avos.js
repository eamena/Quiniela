...
// Each participant block in the 16avos spreadsheet is 5 columns wide:
//   col+0 = home team, col+1 = away team, col+2 = pred_home, col+3 = pred_away, col+4 = empty
const PARTICIPANT_BLOCK_SIZE = 5;

// Use a high row_number offset so 16avos matches never conflict with group-stage rows
const MATCH_ROW_OFFSET = 10000;
...
function importBracket16avos(filePath) {
  ...
  if (!participants.length)
    throw new Error("No participants found in 16avos.xlsx.");

  // Match each participant name against existing DB participants.
  // Strategy: first try exact match, then normalized (accent-stripped, lowercase) match,
  // then manual aliases for participants whose names differ between spreadsheets.
  const NAME_ALIASES = {
    "america cortes": "acg",
  };
  ...
  const tx = db.transaction(() => {
    ...
    for (let rowIndex = 5; rowIndex < rows.length; rowIndex++) {
      ...
      // home_team_id / away_team_id reuse the row_number-based synthetic IDs
      upsertMatch.run(
        rowNumber,
        rowNumber * 2 - 1,
        homeTeam,
...
initDb();
const filePath = process.env.BRACKET_FILE || "16avos.xlsx";
const result = importBracket16avos(filePath);
console.log("16avos import complete:", result);
